...
// import { currency } from "@/lib/currency";
import { CurrencyData } from "@/schemas";
...
const DropdownCurrency = ({
  ...
  useEffect(() => {
    const getCurrencys = async () => {
      const currencyList = await currency();
      if (currencyList) {
        setCurrencyData(currencyList as CurrencyData);
        currencyList?.map((dat) => {
          if (dat?.value === value) {
            setSelected(dat?.value);
