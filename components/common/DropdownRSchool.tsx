...
const DropdownRSchool = ({
  value,
  arrayData,
  onChangeHandler,
  isError,
}: DropdownRSchoolProps) => {
  ...
  useEffect(() => {
    const getSchools = async () => {
      await setSchoolData(arrayData);
      arrayData.map((dat)=>{
        if(dat?.school_id === value){
          setSelected(dat?.school_name);
          setSelected1(dat?.school_id);
        }
      })
    };
    getSchools();
...
export default DropdownRSchool;
