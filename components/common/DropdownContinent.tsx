"use client";
import { Continent } from "@/app/[locale]/(protected)/admin/_components/setups/Continent";
import { useEffect, useRef, useState } from "react";
import { AiOutlineSearch } from "react-icons/ai";
import { BiChevronDown } from "react-icons/bi";

type DropdownContinentProps = {
  value?: string;
  arrayData: Continent[];
  onChangeHandler?: any;
  isError: boolean;
...
const DropdownContinent = ({
  value,
  arrayData,
  onChangeHandler,
  isError,
}: DropdownContinentProps) => {
  ...
  useEffect(() => {
    const getContinents = async () => {
      if(arrayData){
        await setContinentData(arrayData);
        arrayData.map((dat) => {
          if (dat?.continent_id === value) {
            setSelected(dat?.continent_name);
            setSelected1(dat?.continent_id);
          }
        });
      }
      ...
      <div className="w-full font-medium relative">
        ...
        >
          ...
          {continentData?.map((item, index) => (
            ...
            ${
              item?.continent_name?.toLowerCase() === selected?.toLowerCase() &&
              "bg-[var(--clr-secondary)] text-[var(--clr-primary)]"
            }
            ${
              item?.continent_name?.toLowerCase().startsWith(inputValue)
                ? "block"
                : "hidden"
            }`}
              onClick={() => {
                if (
                  ...
                  setSelected(item?.continent_name);
                  setSelected1(item?.continent_id);
                  setOpen(false);
                  setInputValue("");
                }
              }}
...
export default DropdownContinent;
